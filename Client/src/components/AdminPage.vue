<template>
  ...
  <br /><br />
  <div
    class="container-fluid mt-5 adminbackground"
    style="padding-top: 130px; margin-top: 10px"
  >
    <div class="row">
      <h1 class="mb-4">현황 조회</h1>
      <br /><br />
      <!-- Side Navigation -->
      <div class="col-md-3">
        <div
          class="nav flex-column text-center side-nav"
          style="padding-top: 150px; padding-left: 50px"
        >
          ...
          <div class="col-md-6">
            ...
            <div class="dashboard">
              ...
              <p style="font-size: 20px">
                Total Members: {{ totalMembers }}<br /><br />
                Today's New Members: {{ dailyNewMembers }}
              </p>
            </div>
          </div>
          <div class="col-md-6">
            ...
            <table class="table table-hover" style="width: 800px">
              <thead>
                <tr>
                  <th style="width: 200px; background-color: beige">
                    관광지 ID
                  </th>
                  <th style="width: 250px; background-color: beige">
                    관광지명
                  </th>
              ...
              <tbody>
                <template v-if="likesSpot && likesSpot.length > 0">
                  <tr
                    v-for="item in likesSpot.slice(0, 10)"
                    :key="item.contentid"
                  >
...
export default {
  ...
  methods: {
    ...
    fetchMembers() {
      axios
        .get("http://192.168.0.78:3000/api/member")
        .then((response) => {
    ...
    fetchDailyNewMembers() {
      axios
        .get("http://192.168.0.78:3000/api/member")
        .then((response) => {
          this.dailyNewMembers = response.data.filter(
            (member) => member.today === 1
          ).length;
          console.log("Today's new members updated:", this.dailyNewMembers);
        })
        .catch((error) => {
          console.error(
            "There was an error fetching today's new members:",
            error
          );
        });
    },
    ...
    fetchlikeSpot() {
      axios
        .get("http://192.168.0.78:3000/api/localtourplaces") // Changed to the correct GET endpoint
        .then((response) => {
