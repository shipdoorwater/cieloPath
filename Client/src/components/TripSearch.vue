        ...
        <div class="modal-content">
          ...
          <div class="booking-buttons">
            ...
            <a
              ...
              style="background-color: lightgray; width: 130px"
              target="_blank"
              class="btn btn-booking"
              >기차 예약</a
            >
  ...
  <form class="form-layout">
    ...
    <div class="calendar-container">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary">Save CIELO</button>
      </div>
    </div>
  </div>
</div>
...
export default {
  ...
  methods: {
    ...
    if (!this.startDate) {
      ...
      this.endDate = null;
      this.markers = [clickedDate.toISOString().substring(0, 10)]; // ISO 문자열로 저장
      console.log("Start date set:", this.startDate);
    } else if (this.startDate && !this.endDate) {
      if (clickedDate >= this.startDate) {
        this.endDate = clickedDate;
        this.fillDates(this.startDate, this.endDate);
        console.log("End date set:", this.endDate);
      } else {
        this.startDate = clickedDate;
        this.markers = [clickedDate.toISOString().substring(0, 10)];
        console.log("New start date set:", this.startDate);
      }
    } else {
      this.startDate = clickedDate;
      this.endDate = null;
      this.markers = [clickedDate.toISOString().substring(0, 10)];
      console.log("Dates reset with new start date:", this.startDate);
    }
  },
  fillDates(start, end) {
    const dates = [];
