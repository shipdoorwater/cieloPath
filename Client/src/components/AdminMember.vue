        ...
        >
          <div v-for="item in adminItems" :key="item.title" class="nav mb-3">
            <div
              ...
              :style="{
                backgroundColor: item.isHovered ? item.hoverColor : 'transparent',
              }"
            >
              <img
                v-if="item.imgSrc"
      ...
      <div class="col-md-9">
        ...
        <div class="table-responsive">
          <table class="table table-hover">
            <thead>
              <tr>
                <th style="background-color: beige; width: 300px" scope="col">
                  이메일
                </th>
                <th style="background-color: beige; width: 150px" scope="col">
                  이름
                </th>
                <th style="background-color: beige; width: 150px" scope="col">
                  닉네임
                </th>
                <th style="background-color: beige; width: 200px" scope="col">
                  휴대폰번호
                </th>
                <th style="background-color: beige; width: 500px" scope="col">
                  가입일시
                </th>
                <th style="background-color: beige; width: 400px" scope="col">
                  정상여부
                </th>
                <th style="background-color: beige; width: 300px" scope="col">
                  탈퇴
            ...
            <tbody>
              <tr v-for="item in members" :key="item.email_id">
                ...
                <td>
                  <button
                    @click="deleteMember(item.EMAIL_ID)"
  ...
  methods: {
    deleteMember(EMAIL_ID) {
      if (confirm("탈퇴 하시겠습니까?")) {
        axios
          .put(`http://192.168.0.78:3000/api/member/${EMAIL_ID}/disable`)
          .then(() => {
            const index = this.members.findIndex(
              (item) => item.EMAIL_ID === EMAIL_ID
            );
            if (index !== -1) {
