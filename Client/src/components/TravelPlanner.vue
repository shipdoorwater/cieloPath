...
export default {
  ...
  methods: {
    ...
    console.log("Navigating to SelfPlan with", this.days, this.companions, this.style, this.location);
  ...
  created() {
    if (this.$route.query) {
      ...
      if (this.travelInfo.startDate && this.travelInfo.endDate) {
        this.days = this.calculateDays(this.travelInfo.startDate, this.travelInfo.endDate)+1;
